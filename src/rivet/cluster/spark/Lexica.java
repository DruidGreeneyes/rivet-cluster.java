...
public class Lexica {
    ...
    public static final List<String> assignTopicsToDocument(
        ...
        final Comparator<Tuple2<String, RIV>> similarityToDocument = (a,
                b) -> Double.compare(Labels.similarity(a._2, docLex),
    ...
    public static final JavaPairRDD<String, List<Double>> compareDocumentBatch(
        ...
        final JavaPairRDD<String, RIV> rivs = lexDocuments(documents,
                wordLexicon);
        final List<RIV> rivList = rivs.values().collect();
        return rivs.mapValues(
                (r) -> rivList.stream().map((s) -> Labels.similarity(r, s))
    ...
    public static final RIV lexDocument(final JavaRDD<String> document,
            final WordLexicon wordLexicon) {
        ...
        return document.mapToPair((s) -> new Tuple2<>(s, Optional.empty()))
                ...
                .map(RIV::fromString).map(RIV::normalize)
                .map(sfn(RIV.subtractor(meanVector))).reduce(Labels::addLabels);
    }
    ...
    public static final JavaPairRDD<String, RIV> lexDocuments(
        ...
        return documents
                ...
                .mapValues(RIV::fromString).mapValues(RIV::normalize)
